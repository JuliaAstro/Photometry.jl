...
The value returned will be an two arrays corresponding to the estimated background, whose dimensionality will depend on the `dims` keyword. By default, it will return a background with the same shape as the input `data`. 
...
Perform 2D background estimation using the given estimator using boxes and kernels.

This method will estimate backgrounds in meshes of size `box_size`, using a filter kernel of size `kernel_size`. These correspond to the dimension, so for 2D data you could specify (20,) or (20,20) as the box/kernel size, matching with dims=1 for the scalar variant. 
...
[Background Estimators](@ref)
"""
estimate_background(::BackgroundEstimator, ::AbstractArray, ::Tuple, ::Tuple; dims = :)
